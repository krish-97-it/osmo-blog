...
class MetricsItem implements \Sentry\Serializer\EnvelopItems\EnvelopeItemInterface
{
    ...
    public static function seralizeMetric(\Sentry\Metrics\Types\AbstractType $metric) : string
    {
        /**
        ...
        foreach ($metric->getTags() as $key => $value) {
            $tags[] = \preg_replace(self::TAG_KEY_PATTERN, '', $key) . ':' . self::escapeTagValues($value);
        }
        if (!empty($tags)) {
